...
def run():
    ...
    sns.heatmap(transformer_out[0].numpy(), cmap="cividis", xticklabels=False, yticklabels=token_strs, ax=ax2, cbar_kws={"label": "Transformed Value"})
    ax2.set_xlabel("Embedding Dim")
    ax2.set_ylabel("Tokens")
    st.pyplot(fig2)

    # Attention (if captured)
    ...
    st.markdown("## 5️⃣ Output Prediction (Logits)")

    last_token_rep = transformer_out[:, -1, :]
    output_head = torch.nn.Linear(config.embed_dim, config.vocab_size)
    with torch.no_grad():
        logits = output_head(last_token_rep)
        probs = torch.softmax(logits, dim=-1)
        topk = torch.topk(probs, k=5)

    top_ids = topk.indices[0].tolist()
    top_probs = topk.values[0].tolist()
    top_tokens = [tokenizer.tokenizer.id_to_token(id) for id in top_ids]

    st.write("**Top 5 Predicted Tokens:**")
    for i, (tok, prob) in enumerate(zip(top_tokens, top_probs)):
        st.write(f"{i+1}. `{tok}` → {prob:.4f}")
